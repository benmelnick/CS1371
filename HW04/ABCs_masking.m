...
%    output B. (B)
arr1(A) = 1; %placing a 1 at all 0 in arr1; does this by using logical vector A to see if it should place a 1
B = arr1;
...
arr2(C) = arr1(C); %arr1(C) gives even values, put into same positions in arr2
D = arr2;
...
E = A & C; %compares logical values of every element in A and C
%ex: A = [true true false true] C = [false true false true]
...
F = ~A; %returns true values where A is false (~false returns true)

% 7. Use the knowledge obtained from parts 5 and 6 to find a mask where
%    both A and C are false. (G)
G = ~A & ~C; %returns values of true where both A and C are false
