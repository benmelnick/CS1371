...
E = mod(num, 2) ~= 0; %returns true if odd
...
F = ~isequal(vec1, vec2); %returns true if not equal
...
G = ~strcmp(str1, str2); %returns true if not the same
H = strcmpi(str1, str2); %returns true if the same
